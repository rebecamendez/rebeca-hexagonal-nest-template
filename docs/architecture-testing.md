...
Each target writes its artifacts to its own folder under `apps/api/coverage/` (`all/`, `unit/`, `repository/`, `e2e/`), so a run never overwrites another target's results:

- Coverage data: `lcov.info`, `clover.xml`, and the coverage HTML report (`index.html` plus assets). The `html` coverage reporter is required because Jest 30's `lcov` reporter no longer emits the HTML report on its own.
- Test report: `html-report.html`, generated by `jest-html-reporter`. It lists every spec with its full `given/when/should` name — status codes included — and the header annotates each target with what it tests, what it mocks, and what is real (the `additionalInformation` option).

So a team member can open `coverage/unit/html-report.html` in a browser to read the behavior of the unit suite, and `coverage/unit/index.html` to inspect coverage by file.
